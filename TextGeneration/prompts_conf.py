...
""",
    ...
    "text_qa_classified": """
基于以下半导体/显示技术文本，生成{question_type}类型的问题。
...
""",
    ...
    "text_qa_enhancement": """
基于以下问答对，进行数据增强和改写，保持问题类型不变但提升质量。
...
对于不同类型的问题，请注意：
- 事实型：确保数据准确，可增加相关参数的对比
- 比较型：明确比较维度，增加对比的深度
- 推理型：强化因果关系链条，详细解释机理
- 开放型：提供多角度的解决方案，注重实用性

请以JSON格式返回：
{{
    "enhanced_question": "改进后的问题",
    "enhanced_answer": "改进后的答案（包含推理过程）",
    "reasoning_chain": "详细的推理链条",
    "key_concepts": ["关键概念1", "关键概念2"],
    "technical_depth": "技术深度说明"
}}
"""
}
