...
''',
9: """
你是一位半导体显示技术领域的资深专家，擅长从技术文献中提炼核心知识点并生成高质量的问答对。

以下是给你的技术文本内容（可能包含图片引用）：

{markdown_content}

任务要求：
基于上述文本内容，生成多种类型的高质量问答对。你需要生成以下四种类型的问题，每种类型至少生成1个：

1. **事实型问题（Factual）**：15%
   - 获取具体的技术指标、数值、性能参数、工艺条件等
   - 例如：材料的迁移率、阈值电压、载流子浓度等具体数值

2. **比较型问题（Comparative）**：15%
   - 比较不同材料、结构、工艺或方案的优劣
   - 例如：不同结构的性能差异、不同材料的特性对比等

3. **推理型问题（Reasoning）**：50%
   - 探究机制原理、解释现象原因、分析因果关系
   - 需要深入理解技术原理并进行逻辑推理
   - 例如：某种现象的形成机制、性能变化的根本原因等

4. **开放型问题（Open-ended）**：20%
   - 提出优化建议、改进方案、解决策略
   - 需要综合运用知识提出创新性解决方案
   - 例如：如何改善某个技术问题、优化某个工艺流程等
...
输出格式：
请以JSON数组格式返回，每个问答对包含以下字段：
- question_type: 问题类型（factual/comparative/reasoning/open）
- question: 问题内容
- answer: 答案内容
- reasoning: 推理过程（详细说明如何得出答案）
- key_points: 关键知识点列表
- difficulty: 难度级别（basic/intermediate/advanced）
- context: 必要的背景信息（帮助理解问题但不包含答案）

示例输出：
[
  {
    "question_type": "factual",
    "question": "IGZO TFT的典型阈值电压是多少？",
    "answer": "IGZO TFT的典型阈值电压范围为-0.5V到2V，具体数值取决于器件结构和工艺条件。",
    "reasoning": "根据文献报道，IGZO TFT的阈值电压受氧含量、退火温度等因素影响...",
    "key_points": ["阈值电压", "IGZO特性", "工艺影响"],
    "difficulty": "basic",
    "context": "IGZO是一种氧化物半导体材料，广泛应用于显示器件中..."
  },
  {
    "question_type": "reasoning",
    "question": "为什么增加氧分压会导致IGZO薄膜的载流子浓度降低？",
    "answer": "增加氧分压会减少氧空位的形成，而氧空位是IGZO中主要的施主缺陷...",
    "reasoning": "1) 氧空位在IGZO中充当施主，提供自由电子；2) 增加氧分压使氧空位形成能增加...",
    "key_points": ["氧空位", "载流子浓度", "缺陷化学"],
    "difficulty": "advanced",
    "context": "在IGZO薄膜制备过程中，氧分压是关键工艺参数..."
  }
]
...
2123: """
    ...
    "rewrite_prompt": """
你是一位半导体显示技术领域的专家，擅长优化和改写技术问答对，使其更加清晰、准确和专业。
...
任务要求：
请对上述问答对进行优化改写，生成一个更高质量的版本。改写时需要：

1. **问题改写要求**：
   - 保持原问题的核心意图不变
   - 使表述更加清晰、专业
   - 避免歧义，确保问题指向明确
   - 如果是选择题，确保选项设置合理，具有适当的干扰性
...
输出格式：
请以JSON格式返回改写后的问答对，包含以下字段：
{
    "question": "改写后的问题",
    "answer": "优化后的答案",
    "choices": ["选项A", "选项B", "选项C", "选项D"],  // 如果是选择题
    "reasoning": "完善后的推理过程",
    "lecture": "扩充后的背景知识",
    "context": "优化后的上下文信息",
    "improvements": "本次改写的主要改进点"  // 简要说明做了哪些优化
}

示例：
原始问题："IGZO的载流子浓度是多少？"
改写后："在标准制备条件下，IGZO薄膜的本征载流子浓度典型值是多少？"

注意事项：
1. 保持专业性和准确性
2. 不要改变问题的难度级别
3. 确保所有技术信息的正确性
4. 改写后的内容应该更容易理解，但不失专业性
"""
}
