...
''',
9: """
你是一位半导体显示技术领域的资深专家。以下是包含图片信息的markdown文本内容，图片位置以<image>标记，无关图片以<irrelevant>标记。

markdown文本内容:
{markdown_content}

你的任务是基于提供的文本和图片信息，生成高质量的多类型问答对，用于训练多模态模型。请按照以下四种问题类型的比例生成问答：

**问题类型分布：**
1. **事实型问题（25%）**：获取具体的数值、参数、指标、材料特性等客观信息
2. **比较型问题（25%）**：比较不同材料、工艺、结构、性能等的差异和优劣
3. **推理型问题（35%）**：分析机制原理、探究现象原因、解释实验结果等
4. **应用型问题（15%）**：优化建议、改进方法、实际应用场景等

**输出要求：**
请生成3-5个不同类型的问答对，每个问答对包含以下JSON格式：
```json
{
  "question_type": "事实型/比较型/推理型/应用型",
  "question": "具体问题内容",
  "choices": ["A选项", "B选项", "C选项", "D选项"],
  "answer": "正确答案选项（A/B/C/D）",
  "reasoning": "详细的推理过程和解答步骤",
  "lecture": "解答该问题所需的核心知识点",
  "context": "问题背景和必要的技术说明",
  "difficulty": "简单/中等/困难"
}
```
...
请确保生成的问答对能够有效测试学习者对半导体显示技术的理解程度，既有基础知识考查，也有深层次的分析推理。
""",
9.1:
"""
